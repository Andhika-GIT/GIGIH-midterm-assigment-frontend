...
import { Box, Card, CardBody, CardHeader, Center, Divider, Heading, Stack, StackDivider, Text } from '@chakra-ui/react';

import { useParams } from 'react-router-dom';

// utils
import { createComment, getComment, getProduct, getVideoDetail } from '../utils';
...
const VideoDetail = () => {
  ...
  if (isError) {
    return <Error message="Something wrong, to go back to home page and try again" />;
  }
  ...
  return (
    <Box mb={20}>
      <Center d="flex" flexDirection={'column'} gap={50}>
        <Card w={'full'}>
          <CardBody>
            <ReactPlayer url={video.videoURL} width="full" controls="true" style={{ boxShadow: 'rgba(0, 0, 0, 0.35) 0px 5px 15px' }} />
            <Stack mt="6" spacing="3" mb="3">
              <Heading size="md">{video.title}</Heading>
              <Text>{video.description}</Text>
            </Stack>
            <Divider mb="10" />
            {products.length > 0 ? <ProductList products={products} /> : <Error message="This shop has no product" />}
          </CardBody>
        </Card>
